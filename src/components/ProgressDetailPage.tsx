...
export function ProgressDetailPage({
  ...
  const milestoneProgress = Math.min(
    100,
    Math.round((currentDays / period.currentMilestoneDays) * 100),
  );
          ...
          >
            ...
            <View style={styles.detailSection}>
              ...
              <Text style={styles.detailText}>
                Նպատակ՝ {period.currentMilestoneDays} օր: Մնացել է{' '}
                {Math.max(period.currentMilestoneDays - currentDays, 0)} օր:
              </Text>
              {period.completedMilestones.length > 0 && (
                <Text style={styles.detailText}>
                  Ավարտված՝ {period.completedMilestones.join(', ')} օր
                </Text>
              )}
              {isMilestoneComplete && (
                <View style={styles.nextMilestoneBox}>
                  ...
                  <View style={styles.nextMilestoneGrid}>
                    {getNextMilestoneOptions(period.currentMilestoneDays).map((days) => (
                      <Pressable
                        key={days}
                        onPress={() => onSetMilestone(period.id, days)}
                        style={styles.nextMilestoneButton}
                      >
            ...
            <View style={styles.detailSection}>
              ...
              {improvements.map((item) => {
                ...
                return (
                  <View
                    ...
                    style={[styles.healthItem, isUnlocked && styles.healthItemUnlocked]}
                  >
                    <View
                      style={[styles.healthDayBadge, isUnlocked && styles.healthDayBadgeUnlocked]}
                    >
                      <Text
                        style={[styles.healthDayText, isUnlocked && styles.healthDayTextUnlocked]}
                      >
                        {item.day}
                      </Text>
                    </View>
              ...
              <Text style={styles.healthDisclaimer}>
                Սա բժշկական խորհրդատվություն չէ. փոփոխությունները կարող են տարբեր լինել մարդկանց
                մոտ: Աղբյուրների հիմք՝ հանրային առողջապահական ուղեցույցներ:
              </Text>
            </View>
            ...
            <View style={styles.detailSection}>
              ...
              <Text style={styles.detailText}>
                Հիշեցում՝ ամեն օր {formatReminderTime(period.reminderTime)}
              </Text>
              <Text style={styles.detailText}>
                Օրական ծախս՝ {formatMoney(period.dailyCost || 0)}
              </Text>
